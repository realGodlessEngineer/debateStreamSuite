...
- **Caller**: `dock.html` (control — see **The dock** below), `display.html` / `vertical-display.html` (OBS overlays — caller card shows a colored stance chip + amber/red call timer; landscape also has a top-left "Up Next" on-deck pill fed by the queue). Stance colors live in each overlay/dock CSS keyed by `[data-stance="…"]`; the timer's amber/red thresholds mirror `CALLER.TIMER_*`. The dock also has a **Stage** section (topics `<textarea>` + call-in field, each with Update/Show-Hide) driving two persistent overlays: a numbered topic list and a black call-in pill, both set in the bundled `'Futura XBlkCnIt BT'` font (`public/fonts/futura-xblkcnit-bt.ttf`, Impact fallback) with a 4px black `-webkit-text-stroke` + `paint-order: stroke fill`. On **landscape these live on their own page**, `display-topics.html` (1920×1080, own CSS/JS, no theme stylesheet — its colors are fixed and a theme's body gradient would make the source opaque), so the operator can position and toggle them as a separate OBS source; the two share a `.stage-stack` flex column centered across the top, which is what keeps the pill directly beneath a list whose height varies with the topic count. The vertical overlay still carries its own copy inline (topics red/bottom-left, pill centered). Five more landscape OBS pages follow the same one-source-per-overlay pattern, each with its own CSS/JS and no theme stylesheet: `display-scene.html` (full-bleed Starting Soon/BRB/Ending holding card — deliberately opaque, since a see-through holding screen shows whatever the last live source left behind), `display-segment.html` (compact countdown, separate from the scene card so it can sit on top of one), `display-scoreboard.html` (top-right tally, rows colored by the same `[data-stance]` custom properties as the caller chip, rendering only nonzero sides), `display-claim.html` (top band naming the proposition under debate), and `display-ticker.html` (bottom-edge crawl). Each is driven by its own dock section and control module — `js/scene-control.js`, `js/scoreboard-control.js`, `js/claim-control.js`, `js/ticker-control.js` — all reusing the socket exported at `caller-control.js` (`window.CallerControl.socket`) rather than opening their own

  **The dock** (`dock.html`, `dock.css`) is one window: four bands of fixed chrome and a single scrolling area beneath them. Top to bottom: the title bar, the show-config bar, the ON-AIR strip (`js/dock-onair.js` — six items reflecting which overlays are live; clicking one selects the tab that owns it and focuses its control), and `.dock-bar`. `.dock-bar` holds the **live line** — `#currentName`, `#currentPronouns`, `#currentStance`, `#timerDisplay`, `#timerStatus` — and the tablist. Below them `.dock-panels` is the **only element on the page that scrolls**; `body` is `overflow: hidden` and nothing inside the panel is sticky, so a focused control can never end up under the chrome.

  Four tabs, ordered by how often the operator touches each during a live segment: **Callers** (Update Caller form + timer Start/Stop, the queue, and `<details>` disclosures for add-to-queue and the call log) · **Overlays** (topics, call-in, claim, ticker) · **Scene** (scene card, segment countdown) · **Score** (scoreboard). Callers is the default. `js/dock-tabs.js` drives them to the WAI-ARIA APG Tabs pattern — roving tabindex, Left/Right wrapping, Home/End, automatic activation — plus Alt+1…4 from anywhere in the dock, matched on `event.code === 'Digit' + n` rather than `event.key`, because Alt+numpad is the Windows Alt-code input method and its digits arrive with `altKey` set and `key === '1'`. Active tab and open disclosures persist in `localStorage` under `obs-dock-active-tab` and `obs-dock-disclosures`.

  Three things about this layout are load-bearing and easy to undo by accident:

  - **The live line is the only caller state kept out of the tabs.** Who is live and for how long is what the operator reads while working in Overlays, Scene or Score; everything else about a caller is one Alt+1 away. It is deliberately *not* an `aria-live` region — the clock ticks once a second and would be announced once a second — and it holds nothing interactive, so `#clearBtn` (which ends a call, with no undo) cannot be hit from a strip the operator reads rather than aims at. `#timerStatus::before` is the non-colour channel for running/stopped: a pseudo-element survives the `textContent` replacement `caller-control.js` does, and `.active` is a class it already toggles, so no `:has()` and no extra JS. `caller-control.js` writes `No caller live` rather than `-` when idle, because that string is now permanent chrome.
  - **Every panel is in the DOM at parse time and hidden with the `hidden` attribute.** The five control modules build their element maps eagerly at load, so a node absent at load is a node nothing is wired to for the rest of the session. `.dock-panels .tab-content[hidden]` is scoped so `hidden` outranks the wide-dock `display: grid` rule.
  - **`#callerForm` and `#queueForm` each stay whole inside `#tab-callers`.** A `required` control that is hidden at submit time is not focusable, and Chromium then aborts the submit silently. `#startTimerBtn` / `#stopTimerBtn` sit *outside* `#callerForm` and carry `type="button"`: a typeless button inside a form is a submit button.

  Layout keys off `@media`, never `@container` or `:has()` — the CEF build OBS bundles varies by release. Breakpoints: ≥340px gives `.form-row` two columns; ≤419px tightens the tab bar, ON-AIR strip and live line and drops `#timerDisplay` to 18px (below 18.5px bold it stops counting as large text and has to clear 4.5:1, which `#e63946` does only against the band colour `#0a0a0a`); ≥720px moves the live line onto the tab row and lays panel cards out as `repeat(auto-fit, minmax(280px, 1fr))`; ≤560px tall tightens card padding and drops the scoreboard buttons to 32px. Verified in Electron at eleven canvas sizes from 250×900 to 1200×400: zero page scroll in both axes at every one, and every hot control reachable by `document.elementFromPoint` after scrolling the panel — including the two impossible corners (250×400 and 300×400), where the chrome alone cannot fit and the panel scrolls instead.
...
- **Themes**: OBS displays support swappable themes via `public/js/theme-loader.js` — current themes are `godless-engineer` (red/black), `the-bible-guy` (orange/gold + sky blue), `sandstone` (weathered quarried-stone with ochre accents; adds a grain/bevel surface to the caller & title-bar cards via a `body[data-theme="sandstone"]`-scoped block), and `red-chrome` (see **The nameplate** below). Theme files live in `public/css/themes/`. Selection persists in `localStorage`.

  **The nameplate** (`red-chrome`) is the one theme that changes layout rather than only color. Its `body[data-theme="red-chrome"]` block swaps the caller card's gradient panel for `public/images/nameplate-red.png` — a fixed 3:1 plate centered on the stage with its bar at ~75% of canvas height, rising from below the bottom edge. Nothing else is touched: the title bar, "Up Next" pill, scoreboard, claim band and ticker keep their own chrome, re-tinted by the palette variables. The title bar therefore stays bottom-right while the plate is centered, which is fine because the two never share the screen. Three things about it are load-bearing:

  - **The padding is a measured safe area, not styling.** The 2172×724 source is mostly transparent bloom; the only place text may go is the dark face at y 258..414, x 300..1872. `--plate-pad-top/-bottom/-x` inset the flex row to exactly that rect scaled to `--plate-w`, which is also why `background-size` is `100% 100%` rather than `contain`. Replacing the artwork means re-measuring all of them.
  - **Every length is `calc(N * var(--u, 1px))`.** `--u` is `stage-foreground.css`'s design-pixel unit, so on the consolidated stage the plate scales with the canvas; `display.html` and `vertical-display.html` never define it and the `1px` fallback is exact there, both being fixed-size pages. A portrait `@media (max-aspect-ratio: 1/1)` block drops the plate to 980 units so it fits a 1080-wide canvas. The `.has-ticker` lift needs the theme attribute and the class on one compound (`body[data-theme="red-chrome"].has-ticker`) to beat `body.has-ticker .caller-overlay` — an exact specificity tie the theme sheet loses on source order, since theme-loader.js inserts it *ahead* of the page stylesheet.
  - **The face is one line tall.** `.caller-name` is `nowrap` + ellipsis at roughly 23 characters; a name that wrapped would climb through the chrome rail. `.caller-overlay` re-declares `display: flex` because `vertical-display.css` never set it and `justify-content: center` is inert without it. `.caller-accent` is hidden — the stance edge would land in empty bloom 207 units clear of the artwork, and the stance chip still carries the color.

  Separately and for every theme, **a new caller replacing a live one slides the card out and back in** rather than swapping text underneath the operator. `showCallerCard` compares `data.name` against `currentCallerName` in all three display modules. Keyed on the name alone on purpose: `callerUpdate` also fires for a timer start/stop and for pronoun or stance corrections, and re-animating for those would blank the lower third for a second mid-call, so those still update in place. `populateCaller` runs only after the exit resolves, so the outgoing caller leaves under their own name.
